        ...
        {locations.map((location) => {
          ...
          return (
            <div key={`leads-${location.id}`} className="text-center">
              <p className="mb-1 text-[9px] font-medium text-text-muted">{location.label}</p>
              <div className="grid grid-cols-7 justify-items-center gap-x-0.5 gap-y-0.5">
                {Array.from({ length: 21 }, (_, iconIndex) => (
                  <UserRound
                    ...
                    className={`h-3 w-3 ${iconIndex < icons ? LOCATION_META[location.id].iconClass : "text-text-inactive"}`}
                    strokeWidth={2.3}
                  />
                ))}
              </div>
              <p className="mt-1 text-[10px] font-semibold text-text-semibold">{Math.round(location.leads)} leads</p>
            </div>
          );
        })}
      </div>
